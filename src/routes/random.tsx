...
function RandomNumber() {
  ...
  return (
    <div className="p-8 max-w-lg mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Random Number Generator</h1>
      <p className="text-sm text-gray-500 mb-6">Generate random integers within a range.</p>
          ...
          {(
            ...
            <div key={label} className="flex flex-col gap-1">
              ...
              <input
                ...
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
              />
            </div>
          ))}
        </div>
      ...
      {results.length > 0 && (
        <div className="mt-6">
          ...
          {single ? (
            <div
              className="text-7xl font-bold text-center py-12 text-blue-600 bg-blue-50 rounded-xl border border-blue-100 cursor-pointer select-all"
              onClick={copy}
              title="Click to copy"
            >
              {results[0]}
            ...
            <div className="flex flex-wrap gap-2">
              {results.map((num, i) => (
                <span
                  key={i}
                  className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg font-mono text-sm font-semibold border border-blue-100"
                >
                  {num}
                </span>
              ))}
